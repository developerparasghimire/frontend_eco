      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        ...
        <aside className="h-fit space-y-4 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
          ...
          <div className="cart-trust-strip">
            ...
            <span className="cart-trust-item">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="1" y="3" width="15" height="13" rx="2"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
              Free shipping over $500
            </span>
            <span className="cart-trust-item">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
              Easy returns
            </span>
          </div>
        </aside>
...
export default function CartPage() {
  ...
  // Treat unknown as guest so SSR/initial render doesn't bounce — the auth
  // store hydrates synchronously from localStorage on mount.
  if (status === 'authenticated') return <CartView />;
...
function GuestCartView() {
  ...
  const setQty = useGuestCartStore((s) => s.setQty);
  const setInstall = useGuestCartStore((s) => s.setInstall);
  const remove = useGuestCartStore((s) => s.remove);
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        <ul className="space-y-3">
          {lines.map((line) => {
            const lineTotal =
              Number(line.snapshot.discounted_price) * line.quantity +
              (line.include_installation
                ? Number(line.snapshot.installation_fee) * line.quantity
                : 0);
            return (
              <li
                key={line.product}
              ...
              >
                <div className="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-slate-100 sm:h-24 sm:w-24">
                  {line.snapshot.image ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={line.snapshot.image} alt={line.snapshot.name} className="h-full w-full object-cover" />
                  ) : null}
                </div>
                <div className="flex flex-1 flex-col min-w-0">
                  <div className="flex justify-between gap-2">
                  ...
                  <div className="mt-auto flex flex-wrap items-end justify-between gap-2 pt-2">
                    <div className="space-y-1">
                      <div className="inline-flex items-center rounded-lg border border-slate-300 bg-white">
                        ...
                        <span className="w-12 border-x border-slate-200 py-1 text-center text-sm">
                          {line.quantity}
                        </span>
                        <button
                          type="button"
                      ...
                      {Number(line.snapshot.installation_fee) > 0 ? (
                        <label className="flex items-center gap-1.5 text-xs text-slate-500">
                          <input
                            type="checkbox"
                            checked={line.include_installation}
                            onChange={(e) => setInstall(line.product, e.target.checked)}
                            className="h-3.5 w-3.5 rounded border-slate-300 text-green-600"
                          />
                          + Installation
                        </label>
                    ...
                    <div className="text-right">
                      <p className="text-xs text-slate-400">
                        {formatPrice(line.snapshot.discounted_price)} each
                      </p>
                      <p className="text-base font-bold text-slate-900">
        ...
        <aside className="h-fit space-y-4 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
          ...
          <dl className="space-y-2 text-sm">
            ...
            {installation > 0 ? (
              <div className="flex justify-between">
                <dt className="text-slate-600">Installation</dt>
                <dd className="font-medium text-slate-900">{formatPrice(installation)}</dd>
              </div>
            ) : null}
            <div className="flex justify-between border-t border-slate-200 pt-3 text-base">
              <dt className="font-semibold text-slate-900">Total</dt>
              <dd className="font-bold text-green-700">{formatPrice(total)}</dd>
            </div>
